class ProjectsController < ApplicationController
  ...
  def add_user
    @project_user = Project.new(user_id: params[:user_id], project_id: @project.id)
    
    respond_to do |format|
      if @project_user.save
        format.html {redirect_to users_tenant_project_url(id: @project.id, tenant_id: @project.tenant.id), 
          notice: "User was successfuly added to project."
        }
      else
        format.html {redirect_to users_tenant_project_url(id: @project.id, tenant_id: @project.tenant.id),
  ...
  private
    ...
    def verify_tenant
      unless params[:tenant_id] == Tenant.current_tenant_id.to_s
        redirect_to :root, flash: { error: "You are not authorized to access any other than your own" }
      end
    end
end
